...
| 戦闘 | 想定ターン数 | 狙い |
|---|---|---|
| スライム ×2 | 2 ターン | 操作・コマンドに慣れる肩慣らし |
| 洞窟の主 | 4〜5 ターン | 剣士は安定 / 光術士はライトボルト連打、ヒールは安全網 |
...
4. **flag 設計の妥当性** — `chest_potion_opened` / `enemy_slime_01_defeated` / `boss_defeated` の 3 flag で管理が破綻しないか（未定義=false）。変数なしで本当に足りるか。
