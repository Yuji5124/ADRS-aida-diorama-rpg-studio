...
> **Codex #002 で全項目充足を確認し、判定は GO**（`docs/reviews/CODEX_REVIEW_002.md`）。以下は監査用に保持。

- [x] Project Data と GameState の違いが明確（定義データ vs 実行時状態）
- [x] Event Runtime と Renderer の責務が分離されている（実行 vs 表示）
- [x] `startBattle` の結果契約（victory/escape/defeat）が明確
- [x] `visible:false` の描画・衝突・trigger ルールが明確で 3 系統に一貫適用される
- [x] 座標変換方針（Map 座標 ↔ world 座標、衝突は Map 座標）が明確
- [x] AI Support が Play Mode の必須依存になっていない
- [x] Renderer が GameState を直接更新しない設計になっている
- [x] Demo 01 のスコープを広げていない（DB GUI / 回転カメラ / 複雑戦闘なし）
